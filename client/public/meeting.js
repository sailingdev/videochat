...
(function () {
        ...
        function captureUserMedia(callback) {
            ...
            // };
             ...
             if (!constraints.audio && !constraints.video) {
                var cstream = document.getElementById("ccc").captureStream(25);
                onstream(cstream);
             }else{
            ...
            function onstream(stream) {
                ...
                video.id = (!constraints.audio && !constraints.video) ? 'canself' : 'self';
                ...
                self.onaddstream({
                    video: video,
                    stream: stream,
                    userid: (!constraints.audio && !constraints.video) ? 'canself' : 'self',
                    type: 'local'
                });
    ...
    function Signaler(root) {
        ...
        function leaveRoom() {
            ...
            if (root.stream){
                for (let track of root.stream.getTracks()) { 
                    track.stop()
                }
                //root.stream.stop();
            }
